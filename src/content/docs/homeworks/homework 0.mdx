...
it via Gradescope. **Check Piazza for the Gradescope invitation code**. Anyone
with an `@uic.edu` email address can join the course Piazza. All the Gradescope
autograder does is reads `url.txt`, grabs that URL, and checks for your email
address and the static string mentioned above. **Gradescope will only allow
GitHub based submissions** - you must add `url.txt` to your repository and

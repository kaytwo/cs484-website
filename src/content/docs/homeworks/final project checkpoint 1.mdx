...
TLDR: submit [this form](https://forms.gle/FirVbJbCvsNqckmf6) before the deadline.
...
A previous year's project was a costco discount database. Think "price history" from [steamdb](https://steamdb.info/), but for Costco products. A stretch goal would be to build a "discount predictor" that keeps track of recurring discounts for an item, and informs the user when the next expected time is for something to go on sale.

A sticking point with this project is that it requires creating a web scraper for costco deals, which I would *not* recommend you do, but if you have a really interesting idea and it requires a web scraper to get the data (and it isn't that much data / is unlikely to get blocked), then have at it.

For this project, a reasonable deliverable for that would be:
...
[The form is here](https://forms.gle/FirVbJbCvsNqckmf6).
